...
func TestSnpCalcLaunchDigests(t *testing.T) {
	testCases := map[string]struct {
		...
		"success bin 1": {
			...
			// ./sev-snp-measure.py --mode snp:ovmf-hash --ovmf ovmf_img_1.fd
			ovmfHash:  "a58211791a556a630a4319dc9e2ea96cc0e9784dd9f20a4fadf81b26c98d163fcdcb6703884bbbb80d7b1de45b3d84d0",
			vcpuCount: 2,
			// ./sev-snp-measure.py --mode snp --vcpus 2 --ovmf ovmf_img_1.fd --vmm-type ec2 --snp-ovmf-hash a58211791a556a630a4319dc9e2ea96cc0e9784dd9f20a4fadf81b26c98d163fcdcb6703884bbbb80d7b1de45b3d84d0
		...
		"success bin 2": {
			...
			// ./sev-snp-measure.py --mode snp:ovmf-hash --ovmf ovmf_img_2.fd
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			launchDigest, err := snpCalcLaunchDigests(tc.ovmfPath, tc.vcpuCount, hash)
			if tc.wantErr {
				assert.Error(err)
			} else {
				assert.NoError(err)
